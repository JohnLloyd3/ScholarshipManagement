...
if ($survey) {
    $responses = getResponses($pdo, $surveyId);
    $analytics = getSurveyAnalytics($pdo, $surveyId);
}
...
<div class="page-header">
  ...
  <p class="text-muted">View student survey responses and analytics</p>
</div>
...
<?php if ($survey): ?>
  ...
  <div class="page-header" style="margin-bottom:var(--space-xl);">
    ...
    <p class="text-muted"><?= count($responses) ?> response(s) &mdash; <span class="status-badge status-<?= $survey['status'] ?>"><?= ucfirst($survey['status']) ?></span></p>
  </div>
    ...
    <?php foreach ($analytics as $a): ?>
      <div style="border-bottom:1px solid var(--gray-200);padding:var(--space-lg) 0;">
        ...
        <?php if ($a['type'] === 'rating_scale'): ?>
          <p class="text-muted">Average: <strong><?= $a['average'] ?> / 5</strong> (<?= count($a['answers']) ?> responses)</p>
...
<?php else: ?>
  <div class="content-card">
    <div class="empty-state">
      ...
      <p class="empty-state-description">Choose a survey above to view its results.</p>
    </div>
  </div>
<?php endif; ?>
